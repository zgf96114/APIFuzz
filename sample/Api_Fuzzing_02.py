...
APP_PACKAGE_NAME = "com.test.demo_fuzz_native" #Give the app package name you want to test
SCRIPT_FILES = ["target_native.js"] #Give the function/method js hook script that you want to test
...
"""
device control function which simualte the click behavior just as user done(in another thread)
"""
def device_control():
    time.sleep(0.5)
    while True:
            try:
                if instrumentation.fuzz_start_flag == 2:
                    ui_dev(resourceId="com.test.demo_fuzz_native:id/but2").click()
